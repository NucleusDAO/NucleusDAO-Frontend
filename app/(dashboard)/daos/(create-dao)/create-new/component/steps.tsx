...
const Steps = () => {
  ...
  return (
    <div className="dark:bg-[#191919] bg-white p-4 rounded-lg w-[20%]">
        ...
        <Link href={step.url} key={step.title}>
          <div className={cn('flex space-x-3 py-4 items-center text-sm')}>
            <div className='w-fit relative'>
                ...
                <div className={cn('h-8 w-[1px] bg-[#CCCCCC] dark:bg-[#444444] absolute left-4', (currentStepIndex > index || pathname === REVIEW_DAO_URL) && 'bg-primary')} />
                )}
            </div>
            <p className={cn('trans', pathname.startsWith(step.url) ? 'dark:text-white text-[#292929]' : 'dark:text-[#444444] text-[#CCCCCC]')}>{step.title}</p>
          </div>
        </Link>
      ))}
    </div>
