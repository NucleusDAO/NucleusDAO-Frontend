...
const Layout = ({ children }: ILayout) => {
  ...
  const { isLoading, currentDAO, isMember, error, isError } = useContext(EachDaoContext);
    ...
    <div>
      <div className="flex justify-between border-b dark:border-b-[#292929] pb-6 border-b-[#CCCCCC99]">
        ...
        {isConnected && (
          <React.Fragment>
            {isMember && (
              <Button onClick={handleCreateProposal} loading={routing} loadingText="Please wait...">
          ...
          <div className="space-y-6">
            <div className="lg:flex items-center justify-between">
              <div className="flex space-x-4 lg:items-center items-start">
                <img src={currentDAO?.image} alt={currentDAO?.name} className="rounded-lg object-cover h-[50px] w-[50px]" />
                <div className="lg:space-y-1">
                  <h2 className="font-medium text-lg lg:text-2xl text-dark dark:text-white capitalize">{currentDAO?.name}</h2>
                  <CopyToClipboard text={currentDAO?.domain ?? currentDAO?.account} onCopy={() => toast.success('DAO Address copied!')}>
                    <div className="flex space-x-2 mt-1.5 items-center font-light text-sm text-[#888888] w-[60%] lg:w-fit">
                      <p className="truncate">{currentDAO?.domain ?? currentDAO?.account}</p>
                      <Link href={`https://aescan.io/accounts/${currentDAO?.domain ?? currentDAO?.account}`} target="_blank">
                        <ExternalLink className="cursor-pointer" size={18} role="button" />
                      </Link>
                    </div>
                  </CopyToClipboard>
                </div>
              ...
              <div className="flex items-center space-x-4">
                <Dialog>
                  <DialogTrigger asChild>{!isMember && isConnected && <Button className="mt-4 lg:mt-0">Join DAO</Button>}</DialogTrigger>
                  <DialogContent className="dark:bg-gradient-to-r dark:from-[#1E1E1E] dark:via-[#1E1E1E]">
                    <DialogHeader>
                      <DialogTitle className="dark:text-white font-medium py-3 ">Join DAO</DialogTitle>
                      <DialogDescription className="font-light py-2">
                        You have to make a proposal before you can become a member of a DAO. Do you want to make a proposal now?
                      </DialogDescription>
                    </DialogHeader>

                    <Button className="w-full" onClick={handlePropose} loading={routing} loadingText="Please wait...">
                      Propose
                    </Button>
                  </DialogContent>
                </Dialog>
            ...
            <p className="text-[#888888] text-sm leading-[28px]">{capitalizeFirstLetter(currentDAO?.description)}</p>
          </div>
