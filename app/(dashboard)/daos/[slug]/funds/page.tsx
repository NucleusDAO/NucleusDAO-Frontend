...
const EachDaoFunds = () => {
  ...
  const { getAEPrice, transactionHistory, transactionHistoryError, isTransactionHistoryError, isLoadingTransactionHistory } = useContext(ApiContext);
  ...
  if (isTransactionHistoryError) return <ErrorFetchingComponent description={transactionHistoryError?.error?.message} />;
    ...
    <div className="space-y-4">
      {data.length === 0 ? (
        ...
        <>
                ...
                <div className="space-y-1.5">
                  <div className="flex space-x-2 items-center">
                    <p className="dark:text-white text-dark font-bold text-2xl">
                      {convertCurrency(currentDAO?.balance, price).usd} <span className="text-xs font-light">USD</span>
                    </p>
                  ...
                  <p className="text-[#888888] text-xs font-light">{`~${convertCurrency(currentDAO?.balance, price).ae} AE`}</p>
                </div>
              </div>
              {isConnected && <DepositToken />}
            </div>
            <CopyToClipboard text={userURL} onCopy={() => toast.info('URL copied to clipboard!')}>
              <div className="flex space-x-1.5 font-light text-xs">
                <p className="dark:text-white font-normal text-dark">
                  DAO account name: <span className="text-[#888888]">{userURL}</span>
                </p>
          ...
          <div className="dark:bg-gradient-to-r dark:from-[#1E1E1E] dark:via-[#1E1E1E] dark:to-[#252525] p-4 space-y-4 rounded-lg bg-white">
            <h1 className="dark:text-white font-medium text-xl text-dark">Transaction details</h1>

            <DataTable columns={columns(getAEPrice)} data={transactionHistory || []} />
          </div>
        </>
      )}
    </div>
