...
import { capitalizeFirstLetter, getStatus, getTimeDifference } from '@/libs/utils';
...
const EachProposalView = ({ tabs, currentProposal, refetchData }: IEachProposalView) => {
  ...
  const userVote = address ? currentProposal?.votes?.find((vote: { account: string }) => vote?.account === address) : null;
  ...
  const tabViews: IEachTabView = {
    Result: <ProposalResult currentProposal={currentProposal} countdownTime={countdownTime} />,
    Information: <ProposalInfo currentProposal={currentProposal} countdownTime={countdownTime} />,
  };

  return (
    <div className="space-y-6">
      <h1 className="dark:text-white text-dark font-medium text-xl md:text-3xl pt-6">
        {proposalLists.find((proposal: { type: string }) => proposal.type === currentProposal.proposalType)?.title}
      </h1>
      <div className="lg:flex space-x-3 items-center">
        ...
        <div className="flex space-x-3 py-2 lg:py-0">
          ...
          <p className="font-light text-xs md:text-sm dark:text-white text-dark truncate">{currentProposal?.wallet || currentProposal?.proposer}</p>
        </div>
      </div>
      <div className="space-y-4">
        <p className="text-xs md:text-sm text-defaultText">{capitalizeFirstLetter(description?.slice(0, 180))}</p>
        {description?.length > 180 && <>{!showFullProposal && <Button onClick={() => setShowFullProposal(true)}>Read full proposal</Button>}</>}
      </div>
        ...
        <div className="space-y-8">
          {showFullProposal && (
            <div className="text-xs md:text-sm text-defaultText trans space-y-3">
              ...
              {description?.length > 180 && <Button onClick={() => setShowFullProposal(false)}>Show less</Button>}
            </div>
          )}
        ...
        <div className="space-y-8">
          <div className="rounded-lg dark:bg-[#191919] p-8 space-y-4 bg-white">
            <div className="flex justify-between border-b dark:border-[#1E1E1E] pb-4 items-center border-[#CCCCCC99]">
              <h3 className="font-medium text-xl dark:text-white text-dark">Proposal Details</h3>
              <div
            ...
            <div className="rounded-lg dark:bg-[#191919] p-8 space-y-4 bg-white">
              ...
              {getStatus(currentProposal) === 'Pending' && (
                <p className="dark:text-white text-dark text-sm">
                  Voting period has ended. The result will be shown if the proposal reach its quorum and executed
                </p>
              )}
              {getStatus(currentProposal) === 'Succeeded' && (
                <p className="dark:text-white text-dark text-sm">Voting period has ended. The result of the proposal reach its quorum and it's succeeded.</p>
              )}
              {getStatus(currentProposal) === 'Failed' && (
                <p className="dark:text-white text-dark text-sm">Voting period has ended. The result of the proposal did not reach its quorum and it failed.</p>
              )}

              {getStatus(currentProposal) === 'Active' && <VotingProcess currentProposal={currentProposal} />}
            </div>
          )}
          ...
          <div className="rounded-lg dark:bg-[#191919] p-8 space-y-4 bg-white">
            <div className="flex justify-between border-b dark:border-[#1E1E1E] border-[#CCCCCC99] pb-4 items-center">
              ...
              <p className="text-sm flex space-x-1.5">
                ...
                <span className="dark:text-white text-dark">{currentProposal?.totalVote}</span>
